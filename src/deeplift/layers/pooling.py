...
class Pool3D(SingleInputMixin, Node):
    ...
    def _compute_shape(self, input_shape):
        ...
        shape_to_return = [None] #num channels unchanged 
        for (dim_inp_len, dim_kern_width, dim_stride) in\
            zip(input_shape[1:4], self.pool_size, self.strides):
            if (self.padding != PaddingMode.valid):
                #assuming that overhangs are excluded
                shape_to_return.append(
                 1+int((dim_inp_len-dim_kern_width)/dim_stride)) 
        ...
        if (self.data_format == DataFormat.channels_first):
            input_shape = [input_shape[0], input_shape[4],
                           input_shape[1], input_shape[2], 
                           input_shape[3]] 
...
class MaxPool3D(Pool3D):
    ...
    def _build_activation_vars(self, input_act_vars):
        ...
        to_return = tf.nn.max_pool3d(input=input_act_vars,
                             ksize=[1]+list(self.pool_size)+[1],
                             strides=[1]+list(self.strides)+[1],
                             padding=self.padding)
        if (self.data_format == DataFormat.channels_first):
    ...
    def _grad_op(self, out_grad):
        ...
        if (self.data_format == DataFormat.channels_first):
            out_grad = tf.transpose(out_grad, (0,2,3,4,1))            
            orig_input = tf.transpose(orig_input, (0,2,3,4,1))
            orig_output = tf.transpose(orig_output, (0,2,3,4,1))

        to_return = nn_grad.gen_nn_ops.max_pool3d_grad(
                orig_input=orig_input,
                orig_output=orig_output,
                grad=out_grad,
                ksize=[1]+list(self.pool_size)+[1],
        ...
        if (self.data_format == DataFormat.channels_first):
            to_return = tf.transpose(to_return, (0,4,1,2,3))
        return to_return
