...
class Conv3D(Conv):
    ...
    def _compute_shape(self, input_shape):
        ...
        if (input_shape is None):
            ...
            if (self.padding == PaddingMode.valid):
                for (dim_inp_len, dim_kern_width, dim_stride) in\
                    zip(input_shape[1:4], self.kernel.shape[:2], self.strides):
                    #overhangs are excluded
                    shape_to_return.append(
                     1+int((dim_inp_len-dim_kern_width)/dim_stride))
            elif (self.padding == PaddingMode.same):
        ...
        if (self.data_format == DataFormat.channels_first):
            shape_to_return = [shape_to_return[0], shape_to_return[4],
                               shape_to_return[1], shape_to_return[2],
                               shape_to_return[3]]
    ...
    def _compute_conv_without_bias(self, x, kernel):
        conv_without_bias = tf.nn.conv3d(
                             input=x,
                             filter=kernel,
                             strides=[1]+list(self.strides)+[1],
                             padding=self.padding)
    ...
    def _get_mxts_increments_for_inputs(self):
        ...
        if (self.data_format == DataFormat.channels_first):
            pos_mxts = tf.transpose(a=pos_mxts, perm=(0,2,3,4,1))
            neg_mxts = tf.transpose(a=neg_mxts, perm=(0,2,3,4,1))
            inp_diff_ref = tf.transpose(a=inp_diff_ref, perm=(0,2,3,4,1))
            inp_act_vars = tf.transpose(a=inp_act_vars, perm=(0,2,3,4,1))
        ...
        if (self.conv_mxts_mode == ConvMxtsMode.Linear):
            ...
            inp_mxts_increments += zero_inp_mask*tf.nn.conv3d_transpose(
                            value=0.5*(pos_mxts+neg_mxts),
                            filter=self.kernel,
                            output_shape=output_shape,
                            padding=self.padding,
