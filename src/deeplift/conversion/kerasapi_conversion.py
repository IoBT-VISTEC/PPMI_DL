...
def conv3d_conversion(config,
    ...
    if 'dilation_rate' in config:
        if (config['dilation_rate'][0] != 1 or
            config['dilation_rate'][1] != 1 or
            config['dilation_rate'][2] != 1):
    ...
    to_return = [layers.convolutional.Conv3D(
...
def prep_pool3d_kwargs(config, name, verbose):
    return {'name': name,
            'verbose': verbose,
            'pool_size': config[KerasKeys.pool_size],
            'strides': config[KerasKeys.strides],
...
def maxpool3d_conversion(config, name, verbose,
                         maxpool_deeplift_mode, **kwargs):
    pool3d_kwargs = prep_pool3d_kwargs(
                        config=config,
                        name=name,
                        verbose=verbose)
    return [layers.MaxPool3D(
             maxpool_deeplift_mode=maxpool_deeplift_mode,
             **pool3d_kwargs)]
...
def layer_name_to_conversion_function(layer_name):
    name_dict = {
        ...
        'conv3d': conv3d_conversion,
        'maxpooling3d': maxpool3d_conversion,
